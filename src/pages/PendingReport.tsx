  ...
  return (
    <>
      ...
      <div className="space-y-4 p-4 md:p-6">
                ...
                <BarChart data={data.ageBands} margin={{ top: 8, right: 8, bottom: 0, left: -16 }}>
                  ...
                  >
                    {data.ageBands.map((b, i) => (
                      <Cell
                        key={b.name}
                        fill={band && band !== b.name ? '#DCE4F0' : i >= 3 ? '#B45309' : i === 2 ? '#31AAA0' : '#118E85'}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        {!loading && data && (
          <AnalyticsCard
            title="Requirements for information"
            description="Every open RFI across the pending cases in view. Click a card to filter the table, or open a case for its full requirement history."
            bodyClassName="px-0 pb-0"
          >
            <div className="grid grid-cols-2 gap-px border-y border-line bg-line lg:grid-cols-5">
              {[
            ...
            <div className="grid gap-5 px-5 py-4 lg:grid-cols-2">
              {[
                { title: 'Who the requirement sits with', data: rfi.byResponsibility },
                { title: 'Most requested requirements', data: rfi.byRequirement.slice(0, 6) },
              ].map((block) => {
                ...
                return (
                  <div key={block.title}>
                    ...
                    <ul className="space-y-2">
                      {block.data.map((d) => (
                        <li key={d.name}>
                          <div className="flex items-baseline justify-between gap-3">
                            ...
                            <span className="num text-[12px] text-navy-500">
                              {count(d.value)} · {pct(rfi.open ? (d.value / rfi.open) * 100 : 0, 0)}
                            </span>
                          </div>
                          <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-navy-50">
                      ...
                      {block.data.length === 0 && (
                        <li className="py-3 text-[12.5px] text-navy-400">No open requirements in view.</li>
                      )}
                    </ul>
                  </div>
                )
