...
/**
 * Placeholder brand mark for the fictional "Bharat Sentinel Life" identity used
 * throughout this prototype. Drawn inline — no external asset, no real brand.
 */

export function BrandMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} role="img" aria-label="Bharat Sentinel Life">
      <path
...
export function BrandLogo({ className }: { className?: string }) {
  return (
    <span className={cn('inline-flex items-center gap-2 rounded-lg bg-white px-2.5 py-1.5 shadow-sm', className)}>
      ...
      <span className="text-[13px] font-semibold leading-none text-navy-900">Bharat Sentinel Life</span>
    </span>
  )
}
